...
if(!class_exists('\\WPAICG\\WPAICG_Chat')) {
    class WPAICG_Chat
    {
        ...
        public function wpaicg_update_chatbot()
        {
            ...
            if(isset($_REQUEST['bot']) && is_array($_REQUEST['bot'])){
                ...
                $wpdb->query("DELETE FROM ".$wpdb->postmeta." WHERE post_id=".$wpaicg_chatbot_id);
        ...
        public function wpaicg_remove_chat_tokens()
        {
            ...
            if($widget_reset_limit > 0) {
                ...
                $wpdb->query("DELETE FROM " . $wpdb->prefix . "wpaicg_chattokens WHERE source='widget' AND created_at < " . $widget_time);
            }
            if($shortcode_reset_limit > 0) {
                $shortcode_time = time() - ($shortcode_reset_limit * 86400);
                $wpdb->query("DELETE FROM " . $wpdb->prefix . "wpaicg_chattokens WHERE source='shortcode' AND created_at < " . $shortcode_time);
            }
        }
    ...
    PRIMARY KEY  (id)
            ...
            if($wpdb->get_var("SHOW TABLES LIKE '$wpaicgChatTokensTable'") != $wpaicgChatTokensTable) {
                $charset_collate = $wpdb->get_charset_collate();
...
//                $wpaicg_message = urldecode($wpaicg_message);
                ...
                $wpaicg_token_limit_message = 'You have reached your token limit.';
                ...
                if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'wpaicg_chat_shortcode_message'){
                    ...
                    $wpaicg_moderation = $wpaicg_settings['moderation'];
                    $wpaicg_moderation_model = $wpaicg_settings['moderation_model'];
                    $wpaicg_moderation_notice = $wpaicg_settings['moderation_notice'];
                }
                else {
                    $wpaicg_limited_tokens = false;
                    $wpaicg_chat_widget = get_option('wpaicg_chat_widget', []);
                    ...
                    $wpaicg_moderation = isset($wpaicg_chat_widget['moderation']) && $wpaicg_chat_widget['moderation'] ? true : false;
                    $wpaicg_moderation_model = isset($wpaicg_chat_widget['moderation_model']) && !empty($wpaicg_chat_widget['moderation_model']) ? $wpaicg_chat_widget['moderation_model'] : 'text-moderation-latest';
                    $wpaicg_moderation_notice = isset($wpaicg_chat_widget['moderation_notice']) && !empty($wpaicg_chat_widget['moderation_notice']) ? $wpaicg_chat_widget['moderation_notice'] : 'Your message has been flagged as potentially harmful or inappropriate. Please ensure that your messages are respectful and do not contain language or content that could be offensive or harmful to others. Thank you for your cooperation.';
                }
                if(isset($_REQUEST['bot_id']) && !empty($_REQUEST['bot_id'])){
                    ...
                    if($wpaicg_bot) {
                        ...
                        $wpaicg_moderation = isset($wpaicg_chat_widget['moderation']) && !empty($wpaicg_chat_widget['moderation']) ? true : false;
                        $wpaicg_moderation_model = isset($wpaicg_chat_widget['moderation_model']) && !empty($wpaicg_chat_widget['moderation_model']) ? $wpaicg_chat_widget['moderation_model'] : 'text-moderation-latest';
                        $wpaicg_moderation_notice = isset($wpaicg_chat_widget['moderation_notice']) && !empty($wpaicg_chat_widget['moderation_notice']) ? $wpaicg_chat_widget['moderation_notice'] : 'Your message has been flagged as potentially harmful or inappropriate. Please ensure that your messages are respectful and do not contain language or content that could be offensive or harmful to others. Thank you for your cooperation.';
                ...
                if($wpaicg_limited_tokens){
                    if(is_user_logged_in()){
                        $wpaicg_chat_token_log = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."wpaicg_chattokens WHERE source = '".$wpaicg_chat_source."' AND user_id=".get_current_user_id());
                        $wpaicg_token_usage_client = $wpaicg_chat_token_log ? $wpaicg_chat_token_log->tokens : 0;
                    }
                    else{
                        $wpaicg_chat_token_log = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."wpaicg_chattokens WHERE source = '".$wpaicg_chat_source."' AND session_id='".$wpaicg_client_id."'");
                        $wpaicg_token_usage_client = $wpaicg_chat_token_log ? $wpaicg_chat_token_log->tokens : 0;
                    }
                ...
                if(isset($_FILES['audio']) && empty($_FILES['audio']['error'])){
                    ...
                    if(!in_array($filetype['type'], $mime_types)){
                        $wpaicg_result['msg'] = 'We only accept mp3, mp4, mpeg, mpga, m4a, wav, or webm.';
                        wp_send_json($wpaicg_result);
                    }
                    if($file['size'] > 26214400){
                        $wpaicg_result['msg'] = 'Audio file maximum 25MB';
                        wp_send_json($wpaicg_result);
                    }
                ...
                if(!empty($wpaicg_message) && $wpaicg_moderation){
                    ...
                    elseif(isset($moderation_reponse['results']) && is_array($moderation_reponse['results'])){
                        ...
                        if(isset($result['flagged']) && $result['flagged']){
                            ...
                            $wpaicg_result['msg'] = $wpaicg_moderation_notice;
                            $wpaicg_result['status'] = 'error';
...
//                        $wpaicg_result['messages'] = $wpaicg_chatgpt_messages;
                    ...
                    $wpaicg_result['status'] = 'error';
                    $wpaicg_result['msg'] = 'It appears that nothing was inputted.';
                }

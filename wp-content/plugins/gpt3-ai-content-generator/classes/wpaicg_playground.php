...
if(!class_exists('\\WPAICG\\WPAICG_Playground')) {
    class WPAICG_Playground
    {
        ...
        public function wpaicg_token_handling($source)
        {
            global $wpdb;
            $result = array();
            $result['message'] = 'You have reached your token limit.';
            ...
            if($result['limit']){
                if(is_user_logged_in()){
                    $wpaicg_chat_token_log = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix.$result['table']." WHERE  user_id=".get_current_user_id());
                }
                else{
                    $wpaicg_chat_token_log = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix.$result['table']." WHERE session_id='".$wpaicg_client_id."'");
                }
                    ...
                    if ($openai) {
                        ...
                        if(isset($_REQUEST['source_stream']) && in_array($_REQUEST['source_stream'], array('promptbase','form'))){
                            ...
                            if($wpaicg_token_handling['limited']){
                                $has_limited = true;
                                $words = explode(' ',$wpaicg_token_handling['message']);

...
if(!class_exists('\\WPAICG\\WPAICG_Generator')) {
    class WPAICG_Generator{
        ...
        public function wpaicg_content_generator()
        {
            ...
            if(!$openai){
                $this->wpaicg_result['msg'] = 'Missing API Setting';
        ...
        public function init($open_ai, $wpaicg_preview_title = false, $cronjob = false, $post_id = false)
        {
            ...
            if($cronjob){
                ...
                $this->wpaicg_toc_title = get_option('wpaicg_toc_title','Table of Contents');
            ...
            if($step == 'image'){
                ...
                if(!empty($this->wpaicg_image_source)){
                    ...
                    if(!empty($this->wpaicg_result['img'])){
                        ...
                        $half = round($half);
                        $half = $half - 1;
                        $wpaicg_content = explode("</" . $this->wpaicg_heading_tag . ">", $this->wpaicg_result['content']);
                        $wpaicg_content[$half] = $wpaicg_content[$half] . $imgresult;
                        $this->wpaicg_result['content'] = implode("</" . $this->wpaicg_heading_tag . ">", $wpaicg_content);
                    }
                }
            }
            ...
            if ( isset( $complete->error ) ) {
                ...
                if(isset($complete->choices) && is_array($complete->choices)) {
                    ...
                    if(empty($result['data'])){
                        $result['status'] = 'error';
                        $result['msg'] = 'OpenAI returned empty response for this request. Please try again.';
                    }
                    else{
                        $result['length'] = $this->wpaicg_count_words($result['data']);
                    }
                }
                else $result['msg'] = 'OpenAI returned empty response for this request. Please try again.';
            }
            return $result;
        }
    }

...
if(!class_exists('\\WPAICG\\WPAICG_Embeddings')) {
    class WPAICG_Embeddings
    {
        ...
        {
            $wpaicg_result = array('status' => 'error','msg' => 'Missing ID request');
            if(isset($_REQUEST['id']) && !empty($_REQUEST['id'])){
                ...
                if($wpaicg_data){
                    ...
                    $wpaicg_result['msg'] = 'Data not found';
                }
            }
            ...
            <script>
                jQuery(document).ready(function ($){
                    ...
                    $(document).on('click', '.wpaicg-instant-embedding-cancel', function (){
                        ...
                        pendings.find('.wpaicg-instant-embedding-status').html('Cancelled');
                    ...
                    function wpaicgInstantEmbedding(start,ids){
                        ...
                        if(wpaicgInstantWorking) {
                            wpaicgInstantAjax = $.ajax({
                                ...
                                data: {action: 'wpaicg_instant_embedding', id: id},
                                type: 'POST',
                                dataType: 'JSON',
                                success: function (res) {
                                    if (res.status === 'success') {
                                        ...
                                        embedding.find('.wpaicg-instant-embedding-status').html('Indexed');
                                        ...
                                        embedding.find('.wpaicg-instant-embedding-status').html('Error');
                                        embedding.find('.wpaicg-instant-embedding-status').css({
                                            'font-style': 'normal',
                                            'font-weight': 'bold',
                                            'color': '#e30000'
                    ...
                    $('.wpaicg-instan-embedding-btn').click(function (){
                        ...
                        if(ids.length === 0){
                            ...
                            $('.wpaicg_modal_title').html('Instant Embedding<span style="font-weight: bold;font-size: 16px;background: #fba842;padding: 1px 5px;border-radius: 3px;display: inline-block;margin-left: 6px;color: #222;" class="wpaicg-embedding-remain">0/'+ids.length+'</span>');
                            $('.wpaicg_modal').css({
                                top: '5%',
                                height: '90%'
                            })
                            ...
                            html += '<div style="text-align: center"><button class="button button-link-delete wpaicg-instant-embedding-cancel">Cancel</button></div>';
        ...
        public function wpaicg_instant_embedding_button($which)
        {
            ...
            <div class="alignleft actions">
                <a style="height: 32px" href="javascript:void(0)" class="button button-primary wpaicg-instan-embedding-btn">Instant Embedding</a>
            </div>
            <?php
            endif;
        }
        ...
        public function wpaicg_builder_data($wpaicg_data)
        {
            ...
            if (empty($wpaicg_content)) {
                ...
                if($wpaicg_data->post_type == 'post'){
                    $wpaicg_new_content = 'Post Title: '.$wpaicg_data->post_title."\n";
                    $wpaicg_new_content .= 'Post Content: '.$wpaicg_content."\n";
                    $wpaicg_new_content .= 'Post URL: '.get_permalink($wpaicg_data->ID);
                    /*Categories*/
                    ...
                    if($categories_name && is_array($categories_name) && count($categories_name)){
                        $wpaicg_new_content .= "\nPost Categories: ".implode(',',$categories_name);
                    }
                    $wpaicg_content = $wpaicg_new_content;
                }
                /*For Page*/
                if($wpaicg_data->post_type == 'page'){
                    $wpaicg_new_content = 'Page Title: '.$wpaicg_data->post_title."\n";
                    $wpaicg_new_content .= 'Page Content: '.$wpaicg_content."\n";
                    $wpaicg_new_content .= 'Page URL: '.get_permalink($wpaicg_data->ID);
                    $wpaicg_content = $wpaicg_new_content;
                }
                /*For Product*/
                if($wpaicg_data->post_type == 'product' && class_exists('WC_Product_Factory')){
                    ...
                    if($wpaicg_product) {
                        ...
                        if (!empty($product_sku)) {
                            $wpaicg_content_product .= 'Product SKU: ' . $product_sku . "\n";
                        }
                        $product_title = $wpaicg_product->get_title();
                        $wpaicg_content_product .= 'Product Name: ' . $product_title . "\n";
                        $wpaicg_content_product .= 'Product Description: ' . $wpaicg_content . "\n";
                        if(!empty($wpaicg_data->post_excerpt)){
                            $wpaicg_content_product .= 'Product Short Description: ' . $wpaicg_data->post_excerpt . "\n";
                        }
                        $product_url = $wpaicg_product->get_permalink();
                        $wpaicg_content_product .= 'Product URL: ' . $product_url . "\n";
                        $product_regular_price = $wpaicg_product->get_regular_price();
                        if (!empty($product_regular_price)) {
                            $wpaicg_content_product .= "Regular Price: " . $product_regular_price.' '.get_option('woocommerce_currency','USD') . "\n";
                        }
                        $product_sale_price = $wpaicg_product->get_sale_price();
                        if (!empty($product_sale_price)) {
                            $wpaicg_content_product .= 'Sale Price: ' . $product_sale_price.' '.get_option('woocommerce_currency','USD') . "\n";
                        }
                        $product_tax_status = $wpaicg_product->get_tax_status();
                        if (!empty($product_tax_status)) {
                            $wpaicg_content_product .= 'Tax Status: ' . $product_tax_status . "\n";
                        }
                        $product_tax_class = $wpaicg_product->get_tax_class();
                        if (!empty($product_tax_class)) {
                            $wpaicg_content_product .= 'Tax Class: ' . $product_tax_class . "\n";
                        }
                        ...
                        if (!empty($product_external_url)) {
                            $wpaicg_content_product .= 'External Product URL: ' . $product_tax_class . "\n";
                        }
                        $product_shipping_weight = $wpaicg_product->get_weight();
                        if (!empty($product_shipping_weight)) {
                            $wpaicg_content_product .= 'Shipping Weight: ' . $product_shipping_weight .' '.get_option('woocommerce_weight_unit','oz'). "\n";
                        }
                        $product_dimensions = '';
                        ...
                        if (!empty($product_dimensions)) {
                            $wpaicg_content_product .= 'Dimensions: ' . $product_dimensions . "\n";
                        }
                        ...
                        if(isset($stock_status_options[$product_stock_status]) && !empty($stock_status_options[$product_stock_status])){
                            $wpaicg_content_product .= 'Stock Status: '.$stock_status_options[$product_stock_status]."\n";
                        }
                        ...
                        if ($product_attributes && is_array($product_attributes) && count($product_attributes)) {
                            $wpaicg_content_product .= 'Custom Product Attributes: ';

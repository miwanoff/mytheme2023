            ...
            if(
                ...
                if(isset($_REQUEST['wpaicg_product_meta']) && !empty($_REQUEST['wpaicg_product_meta'])){
                    ...
                    if($seo_plugin_activated == '_aioseo_description' && $seo_option){
                        update_post_meta($wpaicgProductID,$seo_plugin_activated,$seo_description);
                        $check = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."aioseo_posts WHERE post_id=".$wpaicgProductID);
                        if($check){
                            $wpdb->query("UPDATE ".$wpdb->prefix."aioseo_posts SET description='".$seo_description."' WHERE post_id=".$wpaicgProductID);
                        }
                        else{
            ...
            if(isset($_REQUEST['step']) && !empty($_REQUEST['step']) && isset($_REQUEST['title']) && !empty($_REQUEST['title'])) {
                ...
                if($wpaicg_woo_custom_prompt) {
                    if($wpaicg_step == 'title'){
                        $wpaicg_languages[$wpaicg_language_key] = get_option('wpaicg_woo_custom_prompt_title', 'Write a SEO friendly product title: %s.');
                    }
                    if($wpaicg_step == 'meta'){
                        $wpaicg_languages[$wpaicg_language_key] = get_option('wpaicg_woo_custom_prompt_meta', 'Write a meta description about: %s. Max: 155 characters.');
                    }
                    if($wpaicg_step == 'short'){
                        $wpaicg_languages[$wpaicg_language_key] = get_option('wpaicg_woo_custom_prompt_short', 'Summarize this product in 2 short sentences: %s.');
                    }
                    if($wpaicg_step == 'description'){
                        $wpaicg_languages[$wpaicg_language_key] = get_option('wpaicg_woo_custom_prompt_description', 'Write a detailed product description about: %s.');
                    }
                    if($wpaicg_step == 'tags'){
                        $wpaicg_languages[$wpaicg_language_key] = get_option('wpaicg_woo_custom_prompt_keywords', 'Suggest keywords for this product: %s.');
                    }
                }
                /*End Custom Prompt*/

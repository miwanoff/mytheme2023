    ...
    class WPAICG_FineTune
    {
        ...
        public function wpaicg_upload_convert()
        {
            ...
            if(
                ...
                else $wpaicg_result['msg'] = 'The file has been removed';

            }
            else{
                $wpaicg_result['msg'] = 'The file does not exist or removed';
            }
            wp_send_json($wpaicg_result);
        }
        ...
        public function wpaicg_data_converter_count()
        {
            ...
            else $wpaicg_result['msg'] = 'Please select least one data to convert';
            wp_send_json($wpaicg_result);
        }
        ...
        public function wpaicg_data_converter()
        {
            ...
            if(
                ...
                try {
                    ...
                    $sql = "SELECT post_title, post_content FROM ".$wpdb->posts." WHERE post_status='publish' AND post_type IN ('".implode("','",$types)."') ORDER BY post_date ASC LIMIT ".$wpaicg_offset.",".$wpaicg_per_page;
        ...
        public function wpaicg_create_finetune()
        {
            ...
            if(isset($_POST['id']) && !empty($_POST['id'])){
                ...
                if($wpaicg_file){
                    ...
                    $wpaicg_result['msg'] = 'File not found';
                }
            }
            wp_send_json($wpaicg_result);
        }
        ...
        public function wpaicg_finetune_upload()
        {
            ...
            if(isset($_FILES['file']) && empty($_FILES['file']['error'])){
                ...
                if($filetype['ext'] !== 'jsonl'){
                    $wpaicg_result['msg'] = 'Only files with the jsonl extension are supported';
                    wp_send_json($wpaicg_result);
                }
            ...
            else $wpaicg_result['msg'] = 'File upload required';
            wp_send_json($wpaicg_result);
        }
        ...
        public function wpaicg_finetune_events()
        {
            ...
            if(isset($_POST['id']) && !empty($_POST['id'])){
                ...
                if($wpaicg_file){
                    ...
                    $wpaicg_result['msg'] = 'Fine Tune not found';
                }
            }
            wp_send_json($wpaicg_result);
        }
            ...
            if(isset($_POST['id']) && !empty($_POST['id'])){
                ...
                if($wpaicg_file){
                    ...
                    if(!empty($ft_model)) {
                        ...
                        $wpaicg_result['msg'] = 'That model does not exist';
                    }
                }
                else{
                    $wpaicg_result['msg'] = 'File not found';
            ...
            if(isset($result->error)){
                ...
                if(isset($result->data) && is_array($result->data) && count($result->data)){
                    ...
                    foreach($result->data as $item){
                        $wpaicgExist[] = $item->id;
                        $wpaicg_check = $wpdb->get_row("SELECT * FROM ".$wpdb->posts." WHERE post_type='wpaicg_finetune' AND post_title='{$item->id}'");
                        if(!$wpaicg_check) {
                            $wpaicg_file_id = wp_insert_post(array(
                                'post_title' => $item->id,
                    ...
                    if(count($wpaicgExist)){
                        $wpdb->query("DELETE FROM ".$wpdb->posts." WHERE post_type='wpaicg_finetune' AND post_title NOT IN ('".implode("','",$wpaicgExist)."')");
                    }
                    else{
        ...
        public function wpaicg_save_files($items)
        {
            ...
            foreach($items as $item){
                if($item->purpose !== 'fine-tune-results' && $item->status != 'deleted') {
                    $wpaicg_check = $wpdb->get_row("SELECT * FROM " . $wpdb->posts . " WHERE post_type='wpaicg_file' AND post_title='{$item->id}'");
                    $wpaicgExist[] = $item->id;
                    if (!$wpaicg_check) {
                        $wpaicg_file_id = wp_insert_post(array(
                            'post_title' => $item->id,
            ...
            if(count($wpaicgExist)) {
                $wpdb->query("DELETE FROM " . $wpdb->posts . " WHERE post_type='wpaicg_file' AND post_title NOT IN ('" . implode("','", $wpaicgExist) . "')");
            }
            else{

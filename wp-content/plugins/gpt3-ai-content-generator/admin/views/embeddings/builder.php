...
if($wpaicg_builder_types && is_array($wpaicg_builder_types) && count($wpaicg_builder_types)) {
    ...
    $wpaicg_total_errors = $wpdb->get_results("SELECT p.ID,p.post_title FROM " . $wpdb->posts . " p LEFT JOIN " . $wpdb->postmeta . " m ON m.post_id = p.ID WHERE p.post_type IN ('".$ids."') AND m.meta_key='wpaicg_indexed' AND m.meta_value='error'");
    $wpaicg_total_skips = $wpdb->get_results("SELECT p.ID,p.post_title FROM " . $wpdb->posts . " p LEFT JOIN " . $wpdb->postmeta . " m ON m.post_id = p.ID WHERE p.post_type IN ('".$ids."') AND m.meta_key='wpaicg_indexed' AND m.meta_value='skip'");
}
?>
<style>
    .wpaicg_modal{

...
<div id="wpaicg-product-generator">
    <p class="wpaicg-form-row">
        <label><strong><?php echo esc_html__('Original Title','wp-ai-content-generator')?></strong></label>
    ...
    <p class="wpaicg-form-row">
        <label for="wpaicg_generate_title"><?php echo esc_html__('Write a SEO friendly product title?','wp-ai-content-generator')?></label>
        <input<?php echo !empty($wpaicg_generate_title) ? ' checked':''?> type="checkbox" value="1" id="wpaicg_generate_title">
    </p>
    <p class="wpaicg-form-row">
        <label for="wpaicg_generate_title"><?php echo esc_html__('Write a SEO Meta Description?','wp-ai-content-generator')?></label>
        <input<?php echo !empty($wpaicg_generate_meta) ? ' checked':''?> type="checkbox" value="1" id="wpaicg_generate_meta">
    </p>
    <p class="wpaicg-form-row">
        <label for="wpaicg_generate_description"><?php echo esc_html__('Write a product description?','wp-ai-content-generator')?></label>
        <input<?php echo !empty($wpaicg_generate_description) ? ' checked':''?> type="checkbox" value="1" id="wpaicg_generate_description">
    </p>
    <p class="wpaicg-form-row">
        <label for="wpaicg_generate_short"><?php echo esc_html__('Write a short product description?','wp-ai-content-generator')?></label>
        <input<?php echo !empty($wpaicg_generate_short) ? ' checked':''?> type="checkbox" value="1" id="wpaicg_generate_short">
    </p>
    <p class="wpaicg-form-row">
        <label for="wpaicg_generate_tags"><?php echo esc_html__('Generate product tags?','wp-ai-content-generator')?></label>
        <input<?php echo !empty($wpaicg_generate_tags) ? ' checked':''?> type="checkbox" value="1" id="wpaicg_generate_tags">
    </p>
    <button type="button" class="button button-primary wpaicg_product_generator_btn"><?php echo esc_html__('Generate','wp-ai-content-generator')?></button>
    <hr>
    <p class="wpaicg-form-row">
        <label><strong><?php echo esc_html__('Product Title','wp-ai-content-generator')?></strong></label>
        <input class="regular-text" name="wpaicg_product_title" id="wpaicg_product_title" type="text">
    </p>
    <p class="wpaicg-form-row">
        <label><strong><?php echo esc_html__('SEO Meta Description','wp-ai-content-generator')?></strong></label>
        <input class="regular-text" name="wpaicg_product_meta" id="wpaicg_product_meta" type="text">
    </p>
    <p class="wpaicg-form-row">
        <label><strong><?php echo esc_html__('Product Short Description','wp-ai-content-generator')?></strong></label>
    </p>
    <p>
        <textarea rows="5" name="wpaicg_product_short" id="wpaicg_product_short"></textarea>
    </p>
    <p class="wpaicg-form-row">
        <label><strong><?php echo esc_html__('Product Description','wp-ai-content-generator')?></strong></label>
    </p>
    <p>
        <textarea rows="10" name="wpaicg_product_description" id="wpaicg_product_description"></textarea>
    </p>
    <p class="wpaicg-form-row">
        <label><strong><?php echo esc_html__('Product Tags','wp-ai-content-generator')?></strong></label>
        <input class="regular-text" name="wpaicg_product_tags" id="wpaicg_product_tags" type="text">
    </p>
    <button style="display: none;" type="button" class="button button-primary wpaicg_product_generator_save"><?php echo esc_html__('Save','wp-ai-content-generator')?></button>
</div>
<script>
    jQuery(document).ready(function ($){
        ...
        function wpaicgProductGenerator(title,step, steps){
            ...
            var data = {'action': 'wpaicg_product_generator', 'step': wpaicg_step,'title' : title}
            if(wpaicg_step === 'title'){
                wpaicg_generatorProcess(wpaicg_step,'Generating Title');
            }
            if(wpaicg_step === 'meta'){
                wpaicg_generatorProcess(wpaicg_step,'Generating Meta Description');
            }
            if(wpaicg_step === 'description'){
                wpaicg_generatorProcess(wpaicg_step,'Generating Description');
            }
            if(wpaicg_step === 'short'){
                wpaicg_generatorProcess(wpaicg_step,'Generating Short Description');
            }
            if(wpaicg_step === 'tags'){
                wpaicg_generatorProcess(wpaicg_step,'Generating Product Tags');
            }
            $.ajax({
                ...
                success: function (res){
                    if(res.status === 'success'){
                        ...
                        wpaicg_generator_process.find('.wpaicg-generating-process-'+wpaicg_step+' span').html('Error');
                ...
                error: function (){
                    ...
                    wpaicg_generator_process.find('.wpaicg-generating-process-'+wpaicg_step+' span').html('Error');
                    wpaicg_ShowError('The server is currently overloaded with other requests. Sorry about that! You can retry your request, or contact us through our help center at help.openai.com if the error persists..', window['wpaicgTimer']);
                    wpaicgRmLoading(wpaicg_product_generator_btn);
                }
            })
        }
        ...
        wpaicg_product_generator_btn.click(function (){
            ...
            if(wpaicg_title === ''){
                alert('<?php echo esc_html__('Please enter product title','wp-ai-content-generator')?>')
            }
            else if(!wpaicgSteps.length){
                alert('<?php echo esc_html__('Please choose least one field','wp-ai-content-generator')?>')
            }
            else{

...
<form action="" method="post" class="wpaicg-audio-form">
    <table class="form-table">
        <tr>
            ...
            <td>
                <select name="purpose" class="regular-text wpaicg-audio-purpose">
                    <option<?php echo $wpaicg_audio_settings['purpose'] == 'transcriptions' ? ' selected':'';?> value="transcriptions">Transcription</option>
                    <option<?php echo $wpaicg_audio_settings['purpose'] == 'translations' ? ' selected':'';?> value="translations">Translation</option>
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <div class="mb-5">
                    <label><input<?php echo $wpaicg_audio_settings['type'] == 'upload' ? ' checked':'';?> class="wpaicg-audio-select" name="type" value="upload" type="radio">&nbsp;Computer</label>&nbsp;&nbsp;&nbsp;&nbsp;
                    <label><input<?php echo $wpaicg_audio_settings['type'] == 'url' ? ' checked':'';?> class="wpaicg-audio-select" name="type" value="url" type="radio">&nbsp;URL</label>
                    <label><input<?php echo $wpaicg_audio_settings['type'] == 'record' ? ' checked':'';?> class="wpaicg-audio-select" name="type" value="record" type="radio">&nbsp;Recording</label>
                </div>
                ...
                <div class="wpaicg-audio-type wpaicg-audio-record" style="<?php echo $wpaicg_audio_settings['type'] == 'record' ? '': 'display:none'?>">
                    <button type="button" class="button button-primary btn-audio-record">Record</button>
                    <button type="button" class="button button-primary btn-audio-record-pause" style="display: none">Pause</button>
                    <button type="button" class="button button-link-delete btn-audio-record-stop" style="display: none">Stop</button>
        ...
        <tr>
            ...
            <td>
                <input type="text" class="regular-text" name="prompt" maxlength="255">
            </td>
        </tr>
        <tr>
        ...
        <tr class="wpaicg_post_type" style="<?php echo !in_array($wpaicg_audio_settings['response'], ['post','page']) ? 'display:none':''?>">
            ...
            <td>
                <select name="status" class="regular-text">
                    <option<?php echo $wpaicg_audio_settings['status'] == 'draft' ? ' selected':'';?> value="draft">Draft</option>
                    <option<?php echo $wpaicg_audio_settings['status'] == 'publish' ? ' selected':'';?> value="publish">Publish</option>
                </select>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <div style="max-width: 400px">
                    <div class="wpaicg_upload_success" style="display: none;margin-bottom: 5px;color: green;">Conversion has completed successfully. You can view the results in the Logs tab.</div>
                    <div class="wpaicg_progress" style="display: none"><span></span><small>Converting.. This will take some time.. Please wait</small></div>
        ...
        <tr>
            <th scope="row"></th>
            <td>
                <button class="button button-primary button-start-converter">Start</button>
                <button style="display: none" class="button button-link-delete wpaicg-btn-cancel" type="button">Cancel</button>
                <button class="button wpaicg-btn-setting" type="button">Set as Default</button>
            </td>
        </tr>
    </table>
</form>
<script>
    jQuery(document).ready(function ($){
        ...
        var wpaicg_audio_mime_types = ['audio/mpeg','video/mp4','video/mpeg','audio/m4a','audio/wav','video/webm'];
        ...
        wpaicg_btn_record_pause.click(function (){
            if(wpaicg_btn_record_pause.hasClass('wpaicg-paused')){
                wpaicg_btn_record_pause.html('Pause');
                wpaicg_btn_record_pause.removeClass('wpaicg-paused');
            }
            else{
                wpaicg_btn_record_pause.html('Continue');
        ...
        wpaicg_btn_record_stop.click(function () {
            ...
            wpaicg_btn_record.html('Re-Record');
        ...
        $('.wpaicg-btn-setting').click(function (){
            ...
            $.ajax({
                ...
                success: function (res){
                    wpaicgRmLoading(btn);
                    alert('The default settings have been successfully saved.');
                },
                error: function (){
                    wpaicgRmLoading(btn);
        ...
        function wpaicgUploadConverter(data){
            ...
            $.ajax({
                ...
                error: function () {
                    ...
                    wpaicg_progress.addClass('wpaicg_error');
                    wpaicg_progress.find('small').html('Error');
                    wpaicg_error_message.html('Please try again');
                    wpaicg_error_message.show();
                }
            })
        }
            ...
            if(type === 'upload'){
                if($('.wpaicg-audio-upload input')[0].files.length === 0){
                    ...
                    if($.inArray(file.type, wpaicg_audio_mime_types) < 0){
                        error_message = 'We only accept mp3, mp4, mpeg, mpga, m4a, wav, or webm.'
                    }

...
$wpaicg_files_sql = "SELECT f.* FROM ".$wpdb->posts." f WHERE f.post_type='wpaicg_convert' AND f.post_status='publish' ORDER BY f.post_date DESC LIMIT ".$wpaicg_files_offset.",".$wpaicg_files_per_page;
...
<form id="wpaicg_data_converter" method="post" action="">
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                <label><input class="wpaicg_converter_data" checked type="checkbox" name="data[]" value="post"> Posts</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <label><input class="wpaicg_converter_data" type="checkbox" name="data[]" value="page"> Pages</label>
                <?php
                if(in_array('product',get_post_types()) && class_exists( 'woocommerce' )):
                    ?>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label><input class="wpaicg_converter_data" type="checkbox" name="data[]" value="product"> Products</label>
                <?php
                endif;
                ?>
            </td>
        ...
        <tr>
            ...
            <td>
                ...
                <button class="button-primary button wpaicg_converter_button">Convert</button>
            </td>
        </tr>
        </tbody>
    </table>
...
<script>
    jQuery(document).ready(function ($){
            ...
            if(!$('.wpaicg_converter_data:checked').length){
                ...
                $.ajax({
                    ...
                    success: function (res){
                        if(res.status === 'success'){
                            if(res.count > 0){
                                wpaicgConverter({action: 'wpaicg_data_converter',types: res.types, total: res.count, page: 1,per_page: 100});
                            }
                            else{
                                progressBar.addClass('wpaicg_error');
                                wpaicgRmLoading(btn);
        ...
        wpaicg_convert_upload.click(function (){
            ...
            var html = '<form id="wpaicg_upload_convert" action="" method="post"><input type="hidden" name="action" value="wpaicg_upload_convert"><input type="hidden" id="wpaicg_upload_convert_index" name="index" value="1"><input id="wpaicg_upload_convert_line" type="hidden" name="line" value="0"><input id="wpaicg_upload_convert_lines" type="hidden" value="'+lines+'"><input type="hidden" name="file" value="'+file+'"><p><label>Purpose</label><select style="width: 100%" name="purpose"><option value="fine-tune">Fine-Tune</option></select></p>';
            html += '<p><label>Model Base</label><select style="width: 100%" name="model"><option value="ada">ada</option><option value="babbage">babbage</option><option value="curie">curie</option><option value="davinci">davinci</option></select></p>';
            html += '<p><label>Custom Name</label><input style="width: 100%" type="text" name="custom"></p>';
            html += '<div class="wpaicg-convert-progress wpaicg-upload-bar"><span></span><small>0%</small></div>';
            html += '<div class="wpaicg-upload-message"></div><p><button style="width: 100%" class="button button-primary" id="wpaicg_create_finetune_btn">Upload</button></p>'
        ...
        function wpaicgFileUpload(data, btn){
            ...
            $.ajax({
                ...
                success: function (res){
                    if(res.status === 'success'){
                        if(res.next === 'DONE'){
                            $('.wpaicg-upload-message').html('Upload successfully');
                            wpaicgRmLoading(btn);

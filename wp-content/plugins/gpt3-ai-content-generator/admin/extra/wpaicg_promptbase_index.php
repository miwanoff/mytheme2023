...
<p class="wpaicg_notice_text">We have over a hundred ready-to-use free prompts. You can easily embed them into your website.
    Choose a prompt from the list below and copy its shortcode onto your page.<br><br>
    Please watch video tutorial <a href="https://www.youtube.com/watch?v=hetYOlR-ms4" target="_blank">here</a>.<br><br>
    Please note that you can only embed <u><b>one form per page</b></u>.<br><br>
    ...
    jQuery(document).ready(function ($){
        ...
        $(document).on('click','.wpaicg-prompt-item .wpaicg-prompt-action-delete',function (e){
            ...
            if(conf){
                ...
                $.post('<?php echo admin_url('admin-ajax.php')?>', {action: 'wpaicg_prompt_delete', id: id});
            }
        });
        ...
        $(document).on('click','.wpaicg-prompt-form .wpaicg-prompt-save-draft', function(e){
            ...
            if(post_content !== ''){
                $.ajax({
                    url: '<?php echo admin_url('admin-ajax.php')?>',
                    data: {title: post_title, content: post_content, action: 'wpaicg_save_draft_post_extra',save_source: 'promptbase'},
                    dataType: 'json',
                    type: 'POST',
                    beforeSend: function (){
                        wpaicgLoading(btn);

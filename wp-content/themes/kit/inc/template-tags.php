 ...
 */
function kit_site_logo($args = array(), $display = true)
{
    ...
    /**
     * Filters the arguments for `kit_site_logo()`.
     *
     * @since Kit 1.0
     *
     ...
     */
    $args = apply_filters('kit_site_logo_args', $args, $defaults);
     ...
     */
    $html = apply_filters('kit_site_logo', $html, $args, $classname, $contents);
 ...
 */
function kit_site_description($display = true)
{
     ...
     */
    $html = apply_filters('kit_site_description', $html, $description, $wrapper);
 ...
 * @return bool
 */
function kit_is_comment_by_post_author($comment = null)
{
 ...
 */
function kit_filter_comment_reply_link($link)
{
...
add_filter('comment_reply_link', 'kit_filter_comment_reply_link');
 ...
 */
function kit_the_post_meta($post_id = null, $location = 'single-top')
{

    echo kit_get_post_meta($post_id, $location); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in kit_get_post_meta().
 ...
 */
function kit_edit_post_link($link, $post_id, $text)
{
    if (is_admin()) {
        return $link;
    }
    ...
    $text = sprintf(
        wp_kses(
            ...
            __('Edit <span class="screen-reader-text">%s</span>', 'kit'),
            array(
                'span' => array(
                    'class' => array(),
                ),
    ...
    return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . kit_get_theme_svg('edit') . '</span><span class="meta-text"><a href="' . esc_url($edit_url) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';

}

add_filter('edit_post_link', 'kit_edit_post_link', 10, 3);
 ...
 */
function kit_get_post_meta($post_id = null, $location = 'single-top')
{
     ...
     */
    $disallowed_post_types = apply_filters('kit_disallowed_post_types_for_meta_output', array('page'));
    ...
    if ('single-top' === $location) {
        ...
        $post_meta = apply_filters(
            'kit_post_meta_location_single_top',
            array(
                'author',
                'post-date',
                'comments',
         ...
         */
        $post_meta = apply_filters(
            'kit_post_meta_location_single_bottom',
            array(
                'tags',
            )
        );
    ...
    if ($post_meta && !in_array('empty', $post_meta, true)) {
         ...
         *
         * @since Kit 1.0
         * @since Kit 1.1 Added the `$post_meta` and `$location` parameters.
         *
         * @param int    $post_id   Post ID.
         ...
         */
        do_action('kit_start_of_post_meta_list', $post_id, $post_meta, $location);
...
/* translators: Hidden accessibility text. */
            ...
            ?>
							...
							<?php kit_the_theme_svg('calendar');?>
						</span>
						<span class="meta-text">
...
/* translators: Hidden accessibility text. */
            ...
            ?>
							...
							<?php kit_the_theme_svg('folder');?>
						</span>
						<span class="meta-text">
							<?php _ex('In', 'A string that is output before one or more categories', 'kit');?> <?php the_category(', ');?>
						</span>
					</li>
					<?php
...
/* translators: Hidden accessibility text. */
        ...
        if (in_array('sticky', $post_meta, true) && is_sticky()) {
            ...
            ?>
					<li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php kit_the_theme_svg('bookmark');?>
						</span>
						<span class="meta-text">
							<?php _e('Sticky post', 'kit');?>
						</span>
					</li>
					<?php
         ...
         *                          Accepts 'single-top' or 'single-bottom'.
         */
        do_action('kit_end_of_post_meta_list', $post_id, $post_meta, $location);
 ...
 */
function kit_filter_wp_list_pages_item_classes($css_class, $page, $depth, $args, $current_page)
{
...
add_filter('page_css_class', 'kit_filter_wp_list_pages_item_classes', 10, 5);
 ...
 */
function kit_add_sub_toggles_to_main_menu($args, $item, $depth)
{
    ...
    if (isset($args->show_toggles) && $args->show_toggles) {
        ...
        if (in_array('menu-item-has-children', $item->classes, true)) {
            ...
            $toggle_duration = kit_toggle_duration();
            ...
            /* translators: Hidden accessibility text. */
            __('Show sub menu', 'kit') .
            '</span>' . kit_get_theme_svg('chevron-down') . '</button>';
...
add_filter('nav_menu_item_args', 'kit_add_sub_toggles_to_main_menu', 10, 3);
 ...
 */
function kit_nav_menu_social_icons($item_output, $item, $depth, $args)
{
    // Change SVG icon inside social links menu if there is supported URL.
    if ('social' === $args->theme_location) {
        $svg = kit_SVG_Icons::get_social_link_svg($item->url);
        if (empty($svg)) {
            $svg = kit_get_theme_svg('link');
        }
...
add_filter('walker_nav_menu_start_el', 'kit_nav_menu_social_icons', 10, 4);
...
add_action('wp_head', 'kit_no_js_class');
 ...
 */
function kit_get_the_archive_title($title)
{
     ...
     */
    $regex = apply_filters(
        'kit_get_the_archive_title_regex',
        array(
            'pattern' => '/(\A[^\:]+\:)/',
...
add_filter('get_the_archive_title', 'kit_get_the_archive_title');
...
function kit_toggle_duration()
{
     ...
     */
    $duration = apply_filters('kit_toggle_duration', 250);
